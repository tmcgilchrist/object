...
let test_elf_header_parsing hello_world_path =
  ...
  Alcotest.(check int) "ELF magic byte 0" 0x7f 
    (Unsigned.UInt8.to_int magic0);
  Alcotest.(check int) "ELF magic byte 1" (Char.code 'E')
    (Unsigned.UInt8.to_int magic1);
  Alcotest.(check int) "ELF magic byte 2" (Char.code 'L')
    (Unsigned.UInt8.to_int magic2);
  Alcotest.(check int) "ELF magic byte 3" (Char.code 'F')
    (Unsigned.UInt8.to_int magic3);
  
  (* Test 64-bit class *)
  Alcotest.(check int) "ELF class (64-bit)" 2
    (Unsigned.UInt8.to_int header.e_ident.elf_class);
  
  (* Test little-endian *)
  Alcotest.(check int) "ELF data encoding (little-endian)" 1
    (Unsigned.UInt8.to_int header.e_ident.elf_data);
  
  (* Test file type (position-independent executable) *)
  Alcotest.(check int) "ELF type (PIE)" 3
    (Unsigned.UInt16.to_int header.e_type);
  
  (* Test machine type (AArch64) *)
  Alcotest.(check int) "ELF machine (AArch64)" 183
    (Unsigned.UInt16.to_int header.e_machine);
  
  (* Test entry point *)
  Alcotest.(check int64) "Entry point" 0x640L
...
let test_elf_sections hello_world_path =
  ...
  (* Test section count *)
  Alcotest.(check bool) "Has multiple sections" true
    (Array.length sections > 20);
  ...
  | Some text_sec ->
      Alcotest.(check int64) ".text section size" 0x138L
        (Unsigned.UInt64.to_int64 text_sec.sh_size);
      Alcotest.(check int64) ".text section address" 0x640L
        (Unsigned.UInt64.to_int64 text_sec.sh_addr);
      Alcotest.(check int) ".text section type (PROGBITS)" 1
  ...
  | Some data_sec ->
      Alcotest.(check int64) ".data section size" 0x10L
        (Unsigned.UInt64.to_int64 data_sec.sh_size);
      Alcotest.(check int64) ".data section address" 0x20000L
  ...
  | Some bss_sec ->
      Alcotest.(check int64) ".bss section size" 0x8L
        (Unsigned.UInt64.to_int64 bss_sec.sh_size);
      Alcotest.(check int) ".bss section type (NOBITS)" 8
  ...
  | Some rodata_sec ->
      Alcotest.(check int64) ".rodata section size" 0x16L
        (Unsigned.UInt64.to_int64 rodata_sec.sh_size);
      Alcotest.(check int64) ".rodata section address" 0x790L
...
let test_elf_program_headers hello_world_path =
  ...
  Alcotest.(check bool) "Has multiple program headers" true
    (Array.length programs >= 8);
  
  (* Find LOAD segments *)
  let load_segments = Array.fold_left (fun acc prog ->
    match prog.p_type with
    | `PT_LOAD -> prog :: acc
    | _ -> acc
  ) [] programs |> List.rev in
  ...
  | first_load :: _ ->
      Alcotest.(check int64) "First LOAD segment virtual address" 0x0L
        (Unsigned.UInt64.to_int64 first_load.p_vaddr);
      Alcotest.(check int64) "First LOAD segment file size" 0x89cL
        (Unsigned.UInt64.to_int64 first_load.p_filesz);
      Alcotest.(check bool) "First LOAD segment is readable and executable" true
  ...
  let dynamic_segment = Array.find_opt (fun prog ->
    match prog.p_type with
    | `PT_DYNAMIC -> true
    | _ -> false
  ) programs in
  ...
  | Some dyn ->
      Alcotest.(check int64) "DYNAMIC segment virtual address" 0x1fda0L
        (Unsigned.UInt64.to_int64 dyn.p_vaddr);
      Alcotest.(check int64) "DYNAMIC segment file size" 0x1f0L
...
let test_elf_symbol_table hello_world_path =
  ...
  let symtab_section = Array.find_opt (fun sec ->
    sec.sh_name_str = ".symtab"
  ) sections in
  
  (match symtab_section with
  | Some symtab ->
      Alcotest.(check int64) ".symtab section size" 0x840L
        (Unsigned.UInt64.to_int64 symtab.sh_size);
      Alcotest.(check int) ".symtab section type (SYMTAB)" 2
        (Unsigned.UInt32.to_int symtab.sh_type)
  | None -> Alcotest.fail "Could not find .symtab section");
  
  (* Find string table section *)
  let strtab_section = Array.find_opt (fun sec ->
    sec.sh_name_str = ".strtab" 
  ) sections in
  
  (match strtab_section with
  | Some strtab ->
      Alcotest.(check int64) ".strtab section size" 0x233L
        (Unsigned.UInt64.to_int64 strtab.sh_size);
      Alcotest.(check int) ".strtab section type (STRTAB)" 3
...
let test_elf_dynamic_section hello_world_path =
  ...
  let dynamic_section = Array.find_opt (fun sec ->
    sec.sh_name_str = ".dynamic"
  ) sections in
  
  (match dynamic_section with
  | Some dyn ->
      Alcotest.(check int64) ".dynamic section size" 0x1f0L
        (Unsigned.UInt64.to_int64 dyn.sh_size);
      Alcotest.(check int64) ".dynamic section address" 0x1fda0L
        (Unsigned.UInt64.to_int64 dyn.sh_addr);
      Alcotest.(check int) ".dynamic section type (DYNAMIC)" 6
...
let test_elf_section_contents hello_world_path =
  ...
  let rodata_section = Array.find_opt (fun sec ->
    sec.sh_name_str = ".rodata"
  ) sections in
  
  (match rodata_section with
  | Some rodata ->
      let content = Bigarray.Array1.sub buffer
        (Unsigned.UInt64.to_int rodata.sh_offset)
        (Unsigned.UInt64.to_int rodata.sh_size) in
      
      (* Check that we can read the content *)
      Alcotest.(check bool) "Can read .rodata content" true
      ...
      for i = 0 to min (Bigarray.Array1.dim content - 13) 12 do
        if Bigarray.Array1.get content i = Char.code 'H' &&
           ...
           Bigarray.Array1.get content (i + 4) = Char.code 'o' then
          has_hello := true
      done;
...
let binary_path =
  ...
  Cmdliner.Arg.(required & opt (some file) None & info ["binary"; "b"] ~doc ~docv:"BINARY")

let () =
  Alcotest.run_with_args "ELF Tests" binary_path [
    ("header", [ ("parse ELF header correctly", `Quick, test_elf_header_parsing) ]);
    ("sections", [ 
      ("parse ELF sections correctly", `Quick, test_elf_sections);
      ("read section contents", `Quick, test_elf_section_contents);
    ]);
    ("program_headers", [ ("parse program headers correctly", `Quick, test_elf_program_headers) ]);
    ("symbol_table", [ ("parse symbol table", `Quick, test_elf_symbol_table) ]);
    ("dynamic", [ ("parse dynamic section", `Quick, test_elf_dynamic_section) ]);
  ]