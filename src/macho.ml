...
let rec decode_flags n acc = function
  ...
  | _ :: flags -> decode_flags n acc flags

let read_header buf =
  let start = buf.position in
  let magic = magic (Unsigned.UInt32.to_int (Read.u32 buf)) in
  let cpu_type = cpu_type (Unsigned.UInt32.to_int (Read.u32 buf)) in
  let cpu_subtype =
    cpu_subtype cpu_type (Unsigned.UInt32.to_int (Read.u32 buf))
  in
  let file_type = file_type (Unsigned.UInt32.to_int (Read.u32 buf)) in
  let _nb_cmds = Read.u32 buf in
  let size_of_cmds = Read.u32 buf in
  let flags =
    decode_flags (Unsigned.UInt32.to_int (Read.u32 buf)) [] header_flags
  in
  let _reserved =
    match magic with
...
let read_n_times f t n = Array.init n (fun _ -> f t)

let read_twolevelhint t =
  let word = Unsigned.UInt32.to_int (Read.u32 t) in
  let isub_image = bits 0 8 word and itoc = bits 8 24 word in
  (Unsigned.UInt32.of_int isub_image, Unsigned.UInt32.of_int itoc)

let read_twolevelhints buf t =
  let offset = Read.u32 t in
  let nhints = Read.u32 t in
  read_n_times read_twolevelhint
    (cursor buf ~at:(Unsigned.UInt32.to_int offset))
    (Unsigned.UInt32.to_int nhints)
...
let rec read_thread t =
  if at_end t then []
    ...
    let state = read_n_times Read.u32 t (Unsigned.UInt32.to_int count) in
    (flavor, state) :: read_thread t

let read_relocation_info _t header ri_address value =
  {
    ri_address;
    ri_symbolnum = Unsigned.UInt32.of_int (bits 0 24 value);
    ri_pcrel = bits 24 1 value = 1;
    ri_length = Unsigned.UInt32.of_int (1 lsl bits 25 2 value);
...
let read_scattered_relocation_info _t header address value =
  {
    ...
    rs_length = Unsigned.UInt32.of_int (1 lsl bits 2 2 address);
    rs_type = reloc_type header.cpu_type (bits 4 4 address);
    rs_address = Unsigned.UInt32.of_int (bits 8 24 address);
    rs_value = value;
  }
...
let read_relocation header t =
  ...
  if Unsigned.UInt32.to_int address land 0x80000000 = 0 then
    `Relocation_info
      (read_relocation_info t header
         (Unsigned.UInt32.to_int address)
         (Unsigned.UInt32.to_int value))
  else
    `Scattered_relocation_info
      (read_scattered_relocation_info t header
         (Unsigned.UInt32.to_int address)
         (Unsigned.UInt32.to_int32 value))
...
let read_module_32 t =
  ...
  let iinit = Unsigned.UInt32.to_int iinit_iterm land 0x0000ffff in
  let iterm = (Unsigned.UInt32.to_int iinit_iterm land 0xffff0000) lsr 16 in
  let ninit_nterm = Read.u32 t in
  let ninit = Unsigned.UInt32.to_int ninit_nterm land 0x0000ffff in
  let nterm = (Unsigned.UInt32.to_int ninit_nterm land 0xffff0000) lsr 16 in
  ...
  let objc_module_info_size = Read.u32 t in
  {
    ...
    dylib_init = (Unsigned.UInt32.of_int iinit, Unsigned.UInt32.of_int ninit);
    dylib_term = (Unsigned.UInt32.of_int iterm, Unsigned.UInt32.of_int nterm);
    dylib_objc_module_info_addr = objc_module_info_addr;
    dylib_objc_module_info_size =
      Unsigned.UInt64.of_int (Unsigned.UInt32.to_int objc_module_info_size);
  }
...
let read_dynamic_symbol_table header t buf =
  ...
  let modtab =
    read_n_times
      ...
      (Unsigned.UInt32.to_int nmodtab)
  in
...
let sec_type flags =
  match flags land 0x000000ff with
  | 0x00 -> `S_REGULAR
  | 0x01 -> `S_ZEROFILL
...
let read_rpath_command buf t =
  let offset = Read.u32 t in
  match
    Read.zero_string (cursor buf ~at:(Unsigned.UInt32.to_int offset)) ()
  with
...
let read_prebound_dylib buf t =
  ...
  let modules =
    read_n_times Read.u8
      (cursor ~at:(Unsigned.UInt32.to_int modulesoff) buf)
      ((Unsigned.UInt32.to_int nmodules / 8)
      + (Unsigned.UInt32.to_int nmodules mod 8))
  in
  (name, modules)
...
let reference_flag_hi16 n =
  decode_flags n []
    [ (0, `REFERENCED_DYNAMICALLY); (2, `SYM_WEAK_REF); (3, `SYM_WEAK_DEF) ]

let reference_flags header v =
  if List.mem `TWOLEVEL header.flags then
    (* Something fishy here, check spec FIXME *)
    [
      reference_flag_lo16 (v land 0xF);
      `LIBRARY_ORDINAL (Unsigned.UInt16.of_int (v land 0xf0));
    ]
  else reference_flag_lo16 (v land 0xF) :: reference_flag_hi16 v
...
let sym_types n =
  ...
  else (true, false, sym_type n, false)

let read_symbol read_value header buf t =
  let sym_name = read_symbol_name t buf in
  let sym_type = Read.u8 t in
  let stabs, sym_pext, sym_type, sym_ext =
    sym_types (Unsigned.UInt8.to_int sym_type)
  in
  let sym_sect = Read.u8 t in
  let sym_flags = Read.u16 t in
  let sym_flags =
    if stabs then `Uninterpreted sym_flags
    else `Flags (reference_flags header (Unsigned.UInt16.to_int sym_flags))
  in
  let sym_value = read_value t in
  { sym_name; sym_type; sym_pext; sym_ext; sym_sect; sym_flags; sym_value }
...
let read_symbol_table header buf t =
  ...
  let f =
    ...
    else fun x -> Unsigned.UInt64.of_uint32 (Read.u32 x)
  in
  let read_symbol = read_symbol f in
  let symcursor = cursor buf ~at:(Unsigned.UInt32.to_int symoff) in
  let symbols =
    read_n_times
      (read_symbol header strsect.buffer)
      symcursor
      (Unsigned.UInt32.to_int nsyms)
  in
...
let read_load_command header buf t =
  ...
  let t = sub t (Unsigned.UInt32.to_int cmdsize - 8) in
  match Unsigned.UInt32.to_int cmd with
...
let read buf =
  ...
  let t = sub t (Unsigned.UInt32.to_int size_of_commands) in
