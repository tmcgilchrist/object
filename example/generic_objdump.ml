...
let print_header (obj : Object.Format.t) =
  ...
  let format_str = match header.format with
    | Object.Format.ELF -> "ELF"
    | Object.Format.MACHO -> "Mach-O"
  in
  let arch_str = match header.architecture with
    | `X86 -> "x86"
    | `X86_64 -> "x86_64"
    | `ARM -> "arm"
    | `ARM64 -> "arm64"
  ...
  (match header.entry_point with
   | Some entry -> Printf.printf "Entry Point: 0x%Lx\n" (Unsigned.UInt64.to_int64 entry)
...
let print_segments (obj : Object.Format.t) =
  ...
  Printf.printf "%-16s %-16s %-16s %-12s %-12s %s\n" "Name" "VirtAddr" "VirtSize" "FileOffset" "FileSize" "Sections";
  Array.iteri (fun _i seg ->
...
let print_sections (obj : Object.Format.t) =
  ...
  Printf.printf "%-3s %-20s %-16s %-16s %-12s %s\n" "Idx" "Name" "Size" "Address" "Offset" "Type";
  Array.iteri (fun i sec ->
    let offset_str = match sec.Object.Format.offset with
...
let dump_file filename =
  try
    ...
    (match Object.Format.find_section obj "__cstring" with
     ...
     | None -> ())
  with
  | exn -> Printf.eprintf "Error processing %s: %s\n" filename (Printexc.to_string exn)
